<template>
  <div class="static grid grid-cols-9 items-center w-full bg-kfe-secondary">
    ...
    <h2 class="col-start-5 text-gray-700 text-xl justify-center ">Patientoversigt </h2>
  ...
  <div class="px-32">
    <ag-grid-vue class="ag-theme-balham w-full h-screen" :columnDefs=" columnDefs " :rowData=" datatable "
      :defaultColDef=" defaultColdDef.def " rowSelection="multiple" @grid-ready=" onGridReady " animateRows="true"
      :pagination=" true " paginationPageSize="100000" @cell-value-changed=" onCellValueChanged "
...
function onCellValueChanged(event: any) {
  ...
  axios.post('http://localhost:5000/patientoversigt', {
    tbl: 'tbl_PatientBehandling', col: event.colDef.field, val: event.value, oldval: event.oldValue, row: '4', RM_ID: 'Kir og Han', Getting_data: 'edit'
  })
...
const columnDefs: Array<any> = [
  ...
  { headername: 'Cpr Nr', field: "Cpr Nr" },
  ...
  { headerName: 'BehandlingStart', field: 'BehandlingStart', filter: 'agDateColumnFilter' },
  { headerName: 'Behandlingstype', field: 'Behandlingstype', filter: 'agNumberColumnFilter', editable: false },
  { headerName: 'Patient Nr', field: "Patient_Nr" },
  { headerName: 'Team', field: 'Diagnosegruppe', editable: false },
  { headerName: 'Bemærkninger', field: 'Bemærkninger', editable: false }
