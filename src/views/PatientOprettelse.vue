<template>
  <div class="grid grid-cols-3">
    <div class="col-span-1 col-start-2">
      <button
        @click="showProtokolForm()"
        class="mt-10 ml-10 mr-1 bg-kfe-primary text-kfe-base hover:text-kfe-primary hover:bg-kfe-base hover:border-kfe-primary border-2 duration-150 cursor-pointer rounded-md py-2 px-3"
      >
        <i v-if="protokol" class="duration-150 fa-solid fa-arrow-right"></i>
        Protokol
        <i v-if="protokol" class="fa-solid fa-arrow-left duration-150"></i>
      </button>
      <button
        @click="showBehandlingForm()"
      ...
      >
        <i
          v-if="protokol != true"
          class="duration-150 fa-solid fa-arrow-right"
        ></i>
        Behandling
        <i
          v-if="protokol != true"
          class="duration-150 fa-solid fa-arrow-left"
        ></i>
      </button>
      <div class="flex flex-col gap-1 justify-center ml-10 w-full">
        ...
        <form>
          ...
          <label for="bom">
            ...
            <input
              ...
              id="bom"
              required
              v-model="alderVedBehandling"
            />
          </label>
        </form>
        <h3 v-if="protokol" class="mt-3 font-medium text-lg">Protokol</h3>
        <h3 v-if="protokol != true" class="mt-3 font-medium text-lg">
          Ordination
        </h3>
        <form>
          <label for="bom">
        ...
        <form>
          <label for="bom"
            >Diagnose:
            <select
              class="rounded dropdown bg-kfe-base h-7"
              v-model="diagnose"
              id="bom"
            >
        ...
        <form>
          <label for="bom">
            ...
            <input
              class="rounded h-7 bg-kfe-base"
              type="date"
              v-model="behandlingsdato"
              id="bom"
              required
            />
          </label>
        ...
        <form v-if="protokol">
          <label for="bom">
            Inkluderet af:
            <input
              class="rounde bg-kfe-base"
              type="text"
              v-model="inkluderetAf"
              id="bom"
              required
            />
          </label>
...
watch(kfeNr, (newKfeNr) => {
  if (newKfeNr.toString().length > 3) {
    axios
      .post("http://localhost:5000/kfe-nr-detaljer", {
