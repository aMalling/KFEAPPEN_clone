    ...
    <div class="col-start-1 col-span-1 self-start">
      <div class="border h-[76vh] truncate shadow-sm pt-1">
        <ul>
          ...
          <p class="px-2 text-sm text-kfe-primary">Fælles:</p>
          <template v-for="item in datatable">
            <li
              v-if="
                item.Diagnosegruppe === 'Fælles' &&
                item.Status_ID >= 4 &&
                item.Status_ID <= 5
              "
              class="truncate px-2 text-sm border-kfe-secondary hover:border-b-2 hover:border-t-2 hover:border-r-2 hover:cursor-pointer"
              @click="onLiCicked(item.KFE_Nr)"
            >
              {{ item.KFE_Nr }} {{ item.Kaldenavn }}
            </li>
...
axios
  .get("http://localhost:5000/kfe-numre")
  .then((response) => {
