  ...
  <div class="w-full justify-center">
    <div class="grid grid-cols-5 items-center w-full bg-kfe-secondary">
      <div class="flex items-center gap-1 ml-32 col-span-2">
        ...
        <input
          class="rounded m-1 bg-kfe-base text-gray-900"
          autofocus
          type="text"
          id="filter-text-box"
      ...
      <h2 class="col-start-3 text-gray-700 text-xl justify-center">
        Inklusionsstatistik {{ date }}
      </h2>
      <div class="col-start-4 cole-span-2 flex">
        <div class="flex content-center col-start-4">
          <button
            class="ml-2 mr-2 my-1 py-1 px-2 text-xs font-bold tracking-wider flex-none bg-kfe-primary text-kfe-base hover:text-kfe-secondary duration-150 cursor-pointer rounded-md"
            @click="restoreFromHardCodedA()"
          >
            Team A
          </button>
          <button
            class="mr-2 my-1 py-1 px-2 text-xs font-bold tracking-wider flex-none bg-kfe-primary text-kfe-base hover:text-kfe-secondary duration-150 cursor-pointer rounded-md"
            @click="restoreFromHardCodedB()"
          >
            Team B
          </button>
          <button
            class="mr-2 my-1 py-1 px-2 text-xs font-bold tracking-wider flex-none bg-kfe-primary text-kfe-base hover:text-kfe-secondary duration-150 cursor-pointer rounded-md"
            @click="restoreFromHardCodedC()"
          >
            Team C
          </button>
          <button
            class="mr-2 my-1 py-1 px-2 text-xs font-bold tracking-wider flex-none bg-kfe-primary text-kfe-base hover:text-kfe-secondary duration-150 cursor-pointer rounded-md"
            @click="restoreFromHardCodedD()"
          >
            Team D
          </button>
          <button
            class="mr-16 my-1 py-1 px-2 text-xs font-bold tracking-wider flex-none bg-kfe-primary text-kfe-base hover:text-kfe-secondary duration-150 cursor-pointer rounded-md"
            @click="restoreFromHardCodedF()"
          >
            Fælles
          </button>
        </div>
        <div class="flex col-start-5">
          <button
            class="ml-[155px] mr-2 my-1 py-1 px-2 text-xs font-bold tracking-wider flex-none bg-kfe-primary text-kfe-base hover:text-kfe-secondary duration-150 cursor-pointer rounded-md"
            @click="resetFilters()"
          >
            Nulstil Filtre
          </button>
    ...
    <div class="px-32">
      <div class="grid grid-cols-4 gap-2 mb-2">
        <div>
          <p class="text-gray-700 italic text-md p-1">
            Patienter inkluderet <span class="font-semibold font">i alt</span>:
          </p>
          <apexchart
            class="border-2"
            type="line"
        ...
        <div>
          <p class="text-gray-700 italic text-md p-1">
            Patienter inkluderet i
            <span class="font-semibold font">firma</span> protokoller:
          </p>
      ...
      <ag-grid-vue
        class="w-full h-[80vh] ag-theme-balham"
...
axios
  .post("http://localhost:5000/inkl_5_aar", {
    RM_ID: loginName.userId,
  })
  .then((response) => {
    //loop igennem response.data og pars med isoparse jeg har lavet
    datatable2.value = response.data;
    inkl_5_aar_data.value = splitarrayOfObjectsToArraysOfObjects(
      response.data,
      "Antal60"
    );
    inkl_5_aar_aar.value = splitarrayOfObjectsToArraysOfObjects(
      response.data,
      "YearStart"
    );
  });
...
function splitarrayOfObjectsToArraysOfObjects(
...
const options: any = ref({
  ...
  xaxis: {
    categories: [
      ...
      current.getFullYear() - 4,
      current.getFullYear() - 3,
      current.getFullYear() - 2,
...
const columnDefs: Array<any> = [
  {
    ...
    cellStyle: { "background-color": "rgb(114, 192, 228, 0.5)" },
  },
